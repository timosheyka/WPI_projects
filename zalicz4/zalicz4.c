...
void dfs(int v, int** a, int* used)
{
    used[v] = 1;
    for (int i = 0; i < arrsize(a[v]); i++)
    {
        if (used[a[v][i]] == 0)
            dfs(a[v][i], a, used);
    }
...
int main()
{
    ...
    {
        a[cur_val[i]] = addsize(a[cur_val[i]]);
        a[cur_val[i]][arrsize(a[cur_val[i]]) - 1] = cur_val[i + 1];
    }
    int* used = calloc((size_t)kiki,(long unsigned int) kiki * sizeof(int));
    for (int i = 0; i < kiki; i++)
        used[i] = 0;
    dfs(start, a, used);
    int ans = 0;
